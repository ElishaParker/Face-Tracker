let model, video, canvas, ctx;
const cursor = document.getElementById('cursor');
let lastBlink = 0;

async function setupCamera() {
  video = document.getElementById('video');
  const stream = await navigator.mediaDevices.getUserMedia({ video: true });
  video.srcObject = stream;
  await new Promise(r => video.onloadedmetadata = r);
}

async function init() {
  await setupCamera();
  canvas = document.getElementById('overlay');
  ctx = canvas.getContext('2d');
  resize();
  window.addEventListener('resize', resize);

  model = await faceLandmarksDetection.load(
    faceLandmarksDetection.SupportedPackages.mediapipeFacemesh
  );

  render();
}

function resize() {
  canvas.width = video.videoWidth;
  canvas.height = video.videoHeight;
}

async function render() {
  const predictions = await model.estimateFaces({ input: video });
  ctx.clearRect(0, 0, canvas.width, canvas.height);

  if (predictions.length > 0) {
    const keypoints = predictions[0].scaledMesh;

    // Draw all points
    ctx.fillStyle = 'rgba(0,255,0,0.5)';
    for (let [x, y] of keypoints) ctx.fillRect(x, y, 1.5, 1.5);

    // Approximate gaze and blink
    const leftEye = keypoints[159]; // upper left eyelid
    const rightEye = keypoints[386]; // upper right eyelid
    const nose = keypoints[1];
    const dx = (rightEye[0] + leftEye[0]) / 2;
    const dy = (rightEye[1] + leftEye[1]) / 2;

    // Cursor mapping
    const x = canvas.width - dx;
    const y = dy;
    cursor.style.left = `${x}px`;
    cursor.style.top = `${y}px`;

    // Simple blink detection: distance between eyelids
    const eyeTop = keypoints[159][1];
    const eyeBottom = keypoints[145][1];
    const eyeDist = Math.abs(eyeTop - eyeBottom);
    if (eyeDist < 2 && Date.now() - lastBlink > 500) {
      lastBlink = Date.now();
      cursor.style.background = 'rgba(255,0,0,0.8)';
      setTimeout(() => cursor.style.background = 'rgba(0,255,0,0.6)', 200);
    }
  }
  requestAnimationFrame(render);
}

init();
